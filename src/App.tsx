...
export default function App() {
  ...
  const canContinue = screen === 1 ? dilemmaSubmitted : screen === 3 ? guidedExpert :
    screen === 4 ? Boolean(marketingQ1Submitted && marketingQ2Submitted && marketingFeedback) :
    screen === 5 ? assessmentSubmitted : screen === 6 ? actionPlan : true;
  ...
  const renderReview = () => {
    ...
    return <>
      ...
      <div className="final-action-plan"><span>YOUR NEXT STEP</span><p>For <strong>{transfer.task}</strong>, watch for <strong>{transfer.risk}</strong> and use this safeguard: <strong>{transfer.safeguard}</strong></p></div>
  ...
  return <div className="app-shell">
    ...
    <main className="course-layout"><aside className="module-nav" aria-label="Module navigation"><span className="nav-eyebrow">MODULE JOURNEY</span>{screenNames.map((name, index) => <button key={name} className={`nav-item ${index === screen ? "active" : ""} ${index < screen || completed ? "visited" : ""}`} type="button" disabled={index > furthest} onClick={() => goTo(index)}><span>{String(index + 1).padStart(2, "0")}</span>{name}{(index < screen || completed) && <Check size={15} aria-hidden="true" />}</button>)}</aside><div className="content-column"><section className="lesson-card">{screens[screen]()}</section>{screen > 0 && <nav className="lesson-pagination" aria-label="Lesson pagination"><button className="secondary-button" type="button" onClick={() => goTo(screen - 1)}><ArrowLeft size={17} /> Previous</button>{screen < screenNames.length - 1 && <button className="primary-button" type="button" disabled={!canContinue} onClick={() => { if (screen === 6) setCompleted(true); goTo(screen + 1); }}>{screen === 6 ? "Finish module" : "Continue"} <ArrowRight size={17} /></button>}</nav>}</div></main>
